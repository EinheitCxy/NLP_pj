            ...
            if args.neftune_alpha is not None:
                if isinstance(model, torch.distributed.fsdp.fully_sharded_data_parallel.FullyShardedDataParallel):
                   ...
                   # new changes
                    ...
                    current_alpha = args.neftune_alpha
                    if args.dynamic_alpha and step_count > 0 and step_count % args.alpha_adjust_interval == 0:
                        current_alpha = adjust_embedding_noise(model, current_alpha, args)
...
# new changes
def adjust_embedding_noise(model, current_alpha, args):
    """Dynamically adjust embedding noise strength"""
    ...
    embedding_norm = torch.norm(embeddings, dim=1)  # Word vector norm
    embedding_std = torch.std(embeddings, dim=1)    # Word vector standard deviation
    ...
    if stability < args.embedding_stability_threshold:
        # Distribution concentrated, increase noise
        current_alpha = min(args.alpha_max, current_alpha + args.alpha_step)
    else:
        # Distribution dispersed, decrease noise
...
if __name__ == '__main__':
    ...
    # new changes   
    parser.add_argument("--dynamic_alpha", action='store_true', help="whether to use dynamic alpha adjustment")
    parser.add_argument("--alpha_min", type=float, default=0.0, help="minimum value for alpha")
    parser.add_argument("--alpha_max", type=float, default=10.0, help="maximum value for alpha")
    parser.add_argument("--alpha_step", type=float, default=0.5, help="step size for alpha adjustment")
    parser.add_argument("--alpha_adjust_interval", type=int, default=100, help="interval (in steps) for alpha adjustment")
    parser.add_argument("--embedding_stability_threshold", type=float, default=0.1, help="threshold for embedding stability")
    parser.add_argument("--embedding_norm_threshold", type=float, default=1.0, help="threshold for embedding norm")
