...
def prune_embedding_matrix(embeddings, pruning_ratio, method='l1', min_threshold=1e-6):
    """Prune the embedding matrix
    
    Args:
        embeddings: Embedding weight matrix
        pruning_ratio: Pruning ratio (0-1) 
        method: Pruning method ('l1' or 'l2')
        min_threshold: Minimum threshold to prevent over-pruning
    
    Returns:
        pruned_embeddings: Pruned embedding matrix
        pruning_stats: Pruning statistics
    """
    try:
        # Input validation
        if not 0 <= pruning_ratio <= 1:
            raise ValueError(f"pruning_ratio must be between 0 and 1, got {pruning_ratio}")
        if method not in ['l1', 'l2']:
            raise ValueError(f"method must be 'l1' or 'l2', got {method}")
        ...
        # Calculate threshold, ensure above minimum
        ...
        total_params = embeddings.numel()
        pruned_params = total_params - torch.sum(mask).item()
        pruning_percentage = (pruned_params / total_params) * 100
        
        # Calculate sparsity
        sparsity = 1 - (torch.sum(mask).item() / total_params)
        
        # Calculate embedding norm change after pruning
        original_norm = torch.norm(embeddings)
        pruned_norm = torch.norm(pruned_embeddings)
        norm_change = ((pruned_norm - original_norm) / original_norm) * 100
        ...
        pruning_stats = {
            ...
            'sparsity': sparsity,
            'norm_change_percentage': norm_change.item()
        }
        
        return pruned_embeddings, pruning_stats
        
    except Exception as e:
        print(f"Error in prune_embedding_matrix: {str(e)}")
        # Return original embeddings on error
        return embeddings, {
            'error': str(e),
            'total_params': embeddings.numel(),
            'pruned_params': 0,
        ...
        for _ in range(accumulation_steps):
            ...
            if args.neftune_alpha is not None:
                if isinstance(model, torch.distributed.fsdp.fully_sharded_data_parallel.FullyShardedDataParallel):
                    ...
                    if args.pruning_ratio > 0 and step_count % args.pruning_interval == 0:
                        try:
                            # Get current embeddings
                            current_embeddings = model._fsdp_wrapped_module.model.embed_tokens.weight
                            
                            # Apply pruning
                            pruned_embeddings, pruning_stats = prune_embedding_matrix(
                                current_embeddings, 
                                args.pruning_ratio,
                                args.pruning_method
                            )
                            ...
                            model._fsdp_wrapped_module.model.embed_tokens.weight.data = pruned_embeddings
                            
                            # # Log pruning statistics, for test
                            # if rank == 0:
                            #     print(f"\nPruning Statistics at step {step_count}:")
                            #     print(f"Total parameters: {pruning_stats['total_params']}")
                            #     print(f"Pruned parameters: {pruning_stats['pruned_params']}")
                            #     print(f"Pruning percentage: {pruning_stats['pruning_percentage']:.2f}%")
                            #     print(f"Pruning threshold: {pruning_stats['threshold']:.4f}")
                            #     print(f"Sparsity: {pruning_stats['sparsity']:.2f}")
                            #     print(f"Norm change: {pruning_stats['norm_change_percentage']:.2f}%\n")
                                
                            #     if args.wandb:
                            #         wandb.log({
                            #             "pruning/stats": pruning_stats,
                            #             "pruning/step": step_count
                            #         })
                                    
                        except Exception as e:
                            print(f"Error during pruning at step {step_count}: {str(e)}")
                            # Continue training without pruning on error
                            pass
...
if __name__ == '__main__':
    ...
    # new changes
    parser.add_argument("--pruning_ratio", type=float, default=0.5, help="ratio for pruning")
    parser.add_argument("--pruning_method", type=str, default="l1", help="method for pruning")
    parser.add_argument("--pruning_interval", type=int, default=100, help="interval for pruning")
